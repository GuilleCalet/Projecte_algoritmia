...
void RadixTrie::insert(const string& word, WordID id, int line, long long pos) {
    ...
    while (i < word.size()) {
        ...
        size_t k = 0;
        const size_t m = min(label.size(), word.size() - i);
...
vector<int> RadixTrie::explore_subtree(const string& prefix) const {
    vector<int> wordIDs;
    Node* node = root;
    ++visited_nodes_;
    size_t i = 0;
    ...
    while (i < prefix.size()) {
        char ch = prefix[i];
        if (node->edges.find(ch) == node->edges.end()) return {};  // El prefijo no se encuentra en el RadixTrie
        size_t k = min(prefix.size() - i, node->edges[ch]->label.size());
        if (prefix.substr(i, k) != node->edges[ch]->label) return {};  // El prefijo no coincide
        i += k;
    ...
    while (!nodes.empty()) {
        ...
        for (auto& edge : currentNode->edges) nodes.push(edge.second->child);
    }
...
vector<WordID> RadixTrie::complete_prefix_topk(const string& prefix, size_t k) const {
    vector<WordID> out;
    visited_nodes_ = 0;
    ...
    // 1) Descenso por el prefijo (permitiendo terminar a mitad de etiqueta)
    size_t i = 0;
    size_t carry = 0; // nº de caracteres que faltan para “llegar” al siguiente nodo
    while (i < prefix.size()) {
        char ch = prefix[i];
        auto it = node->edges.find(ch);
        ...
        const Edge* e = it->second;
        const string& lab = e->label;

        size_t m = min(lab.size(), prefix.size() - i);
        size_t kcmp = 0;
        while (kcmp < m && prefix[i + kcmp] == lab[kcmp]) ++kcmp;
        if (kcmp < m) return out; // no coincide el prefijo
    ...
    // 2) Min-heap por profundidad de caracteres desde el prefijo.
    //    Estado: (depth, rem, node). Mientras rem>0, NO podemos ni leer wordID ni expandir.
    struct Item {
        size_t depth;      // caracteres añadidos desde el prefijo
        size_t rem;        // caracteres que faltan para “alcanzar” el nodo (si venimos desde mitad de label)
        const Node* n;
    ...
    while (!pq.empty() && out.size() < k) {
        ...
        if (it.rem > 0) {
            // Aún estamos “consumiendo” una etiqueta comprimida: gastamos 1 carácter
            pq.push({it.depth + 1, it.rem - 1, it.n});
            continue;
        }

        // Ya hemos “llegado” al nodo lógico
        if (it.depth >= 1 && it.n->wordID != -1) {
            out.push_back(it.n->wordID);          // profundidad exacta en caracteres
            if (out.size() >= k) break;
        }

        // Expandir hijos: cada arista añade label.size() caracteres
        for (const auto& kv : it.n->edges) {
            const Edge* e = kv.second;
            size_t L = e->label.size();
